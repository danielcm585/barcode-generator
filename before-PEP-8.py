...
def flip_bits(s: str) -> str:
    ...
    ret = ''
    for i in s: ret += '0' if i == '1' else '1'
    return ret

encode_key = ['LLLLLL','LLGLGG','LLGGLG','LLGGGL','LGLLGG','LGGLLG','LGGGLL','LGLGLG','LGLGGL','LGGLGL']

encode_bit = {}
encode_bit['L'] = ['0001101','0011001','0010011','0111101','0100011','0110001','0101111','0111011','0110111','0001011']
...
def check_sum(code: str) -> str:
    ...
    for i in range(len(code)):
        digit = int(code[i])
        if (i%2 == 0): digit_sum += digit
        else: digit_sum += 3*digit

    if (digit_sum%10 == 0): return '0'
...
class Barcode:
    def __init__(self):
        ...
        file_name_label = Label(window, text='Save barcode to PS file [eg: EAN13.eps]:', font=self.font_style_bold)
        file_name_label.pack()
        
        self.file_name_var = StringVar()
        file_name_entry = Entry(window, textvariable=self.file_name_var, width=16, font=self.font_style)
        file_name_entry.pack()

        code_label = Label(window, text='Enter code (first 12 decimal digits):', font=self.font_style_bold)
        code_label.pack()
        
        self.code_var = StringVar()
        code_entry = Entry(window, textvariable=self.code_var, width=16, font=self.font_style)
        code_entry.pack()
    ...
    def create_barcode(self):
        ...
        self.canvas.create_text(173, 75, text='EAN-13 Barcode:', font=self.font_style_bold)
        ...
        for i in (range(len(bit_str))):
            if (i == 0): 
                self.canvas.create_rectangle(pos_x, pos_y, pos_x+size_x, pos_y+size_y+10, fill='blue', outline='')
                pos_x += 2*size_x
                self.canvas.create_rectangle(pos_x, pos_y, pos_x+size_x, pos_y+size_y+10, fill='blue', outline='')
                pos_x += size_x
            if (i == 42):
                ...
                pos_x += 2*size_x
            if (bit_str[i] == '1'): 
                self.canvas.create_rectangle(pos_x, pos_y, pos_x+size_x, pos_y+size_y, fill='green', outline='')
            pos_x += size_x 
            if (i == len(bit_str)-1): 
                self.canvas.create_rectangle(pos_x, pos_y, pos_x+size_x, pos_y+size_y+10, fill='blue', outline='')
                pos_x += 2*size_x
        ...
        for i in range(len(self.code)):
            self.canvas.create_text(pos_x, pos_y, text=self.code[i], font=self.font_style_bold)
            if (i == 0 or i == 6): pos_x += size_x
            pos_x += size_x 

        self.canvas.create_text(170, 345, text=f'Check Digit: {lst_digit}', font=self.font_style_bold, fill='orange')
